...
a. Where is closure used in this code? How can you tell? **Answer:** It is used in the return of `personalDice` into the assigned variable (in this case, `dansRoll` and `zoesRoll`). We can tell because `personalDice` returns a function along with its lexical environment. In this case, it isn't particularly useful one since there's no need to preserve state (i.e. our subsequent rolls are independent of the previous ones) other than the person's name.
...
c. What is the lexical scope of `newRoll`? **Answer:** `newRoll` is scoped within the code block when each variable is set equal to `personalDice(<someName>)` though in this case the state of `newRoll` is fully reset with every call. The `name` however is preserved from the assignment of `dansRoll` and `zoesRoll`. 
